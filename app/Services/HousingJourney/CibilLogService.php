...
class CibilLogService extends Service
{
    use CrmTrait;
    /**
     * Export Log.
    ...
    public function exportLog(Request $request)
    {
        ...
        try {
            ...
            $repository = new CibilLogRepository();
            $datas['methodName'] = 'getLog';
            $datas['fileName'] = 'Cibil-Log-Report-';
            $datas['moduleName'] = 'Cibil-Log';
            return $this->exportData($request, $repository, $datas);
        } catch (Throwable  | ClientException $throwable) {
            Log::info("CibilLogService  exportLog" . $throwable->__toString());
    ...
    public function getFilterData()
    {
        try {
            ...
            throw new Throwable(
                Log::info("Service : CibilLogService , Method : getFilterData : %s", $throwable->__toString())
            );
        }
    }
    /**
     ...
     * @return mixed
     */
    public function getLogList(Request $request, CibilLogRepository $cibilLogRepo): mixed
    {
        try {
            $logsList = $cibilLogRepo->getLog($request);
            return $this->responseJson(
                config('crm/http-status.success.status'),
                config('crm/http-status.success.message'),
                config('crm/http-status.success.code'),
            ...
            throw new Throwable(
                Log::info("Service : CibilLogService , Method : getLogList : %s", $throwable->__toString())
            );
        }
    }
}
