...
class OtpLogService extends Service
{
    use CrmTrait;
    /**
     * Export Log.
    ...
    public function exportLog(Request $request)
    {
        ...
        try {
            ...
            $repository = new OtpLogRepository();
            $datas['methodName'] = 'getLog';
            $datas['fileName'] = 'Otp-Log-Report-';
            $datas['moduleName'] = 'Otp-Log';
            return $this->exportData($request, $repository, $datas);
        } catch (Throwable  | ClientException $throwable) {
            Log::info("OtpLogService  exportLog" . $throwable->__toString());
    ...
    public function getFilterData()
    {
        try {
            ...
            throw new Throwable(
                Log::info("Service : OtpLogService , Method : getFilterData : %s", $throwable->__toString())
            );
        }
    }
    /**
     ...
     * @return mixed
     */
    public function getLogList(Request $request, OtpLogRepository $otpLogRepo): mixed
    {
        try {
            $logsList = $otpLogRepo->getLog($request);
            return $this->responseJson(
                config('crm/http-status.success.status'),
                config('crm/http-status.success.message'),
                config('crm/http-status.success.code'),
            ...
            throw new Throwable(
                Log::info("Service : OtpLogService , Method : getLogList : %s", $throwable->__toString())
            );
        }
    }
}
