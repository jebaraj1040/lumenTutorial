...
use App\Repositories\HousingJourney\TalismaCreateContactLogRepository;

class TalismaCreateContactLogService extends Service
{
    use CrmTrait;
    /**
     * Export Log.
    ...
    public function exportLog(Request $request)
    {
        ...
        try {
            ...
            $repository = new TalismaCreateContactLogRepository();
            $datas['methodName'] = 'getLog';
            $datas['fileName'] = 'Talisma-Create-Contact-Log-Report-';
            $datas['moduleName'] = 'Talisma-Create-Contact-Log';
            return $this->exportData($request, $repository, $datas);
        } catch (Throwable  | ClientException $throwable) {
            Log::info("TalismaCreateContactLogService  exportLog" . $throwable->__toString());
    ...
    public function getFilterData()
    {
        try {
            ...
            throw new Throwable(
                Log::info("Service : TalismaCreateContactLogService , Method : getFilterData : %s", $throwable->__toString())
            );
        }
    }
    /**
     ...
     * @return mixed
     */
    public function getLogList(Request $request, TalismaCreateContactLogRepository $talismaLogRepo): mixed
    {
        try {
            ...
            throw new Throwable(
                Log::info("Service : TalismaCreateContactLogService , Method : getLogList : %s", $throwable->__toString())
            );
        }
    }
}
