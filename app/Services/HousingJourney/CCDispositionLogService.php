...
use App\Repositories\HousingJourney\CCDispositionLogRepository;

class CCDispositionLogService extends Service
{
    use CrmTrait;
    /**
     * Export Log.
    ...
    public function exportLog(Request $request)
    {
        ...
        try {
            ...
            $repository = new CCDispositionLogRepository();
            $datas['methodName'] = 'getLog';
            $datas['fileName'] = 'CC-Disposition-Log-Report-';
            $datas['moduleName'] = 'CCDispositionLogService';
            return $this->exportData($request, $repository, $datas);
        } catch (Throwable  | ClientException $throwable) {
            Log::info("CCDispositionLogService  exportLog" . $throwable->__toString());
    ...
    public function getFilterData()
    {
        try {
            ...
            throw new Throwable(
                Log::info("Service : CCDispositionLogService , Method : getFilterData : %s", $throwable->__toString())
            );
        }
    }
    /**
     ...
     * @return mixed
     */
    public function getLogList(Request $request, CCDispositionLogRepository $ccDispositionLogRepo): mixed
    {
        try {
            $logsList = $ccDispositionLogRepo->getLog($request);
            return $this->responseJson(
                config('crm/http-status.success.status'),
                config('crm/http-status.success.message'),
                config('crm/http-status.success.code'),
            ...
            throw new Throwable(
                Log::info("Service : CCDispositionLogService , Method : getLogList : %s", $throwable->__toString())
            );
        }
    }
}
