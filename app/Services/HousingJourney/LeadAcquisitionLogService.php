...
use App\Repositories\HousingJourney\LeadAcquisitionLogRepository;

class LeadAcquisitionLogService extends Service
{
    use CrmTrait;
    /**
     * Export Log.
    ...
    public function exportLog(Request $request)
    {
        ...
        try {
            ...
            $repository = new LeadAcquisitionLogRepository();
            $datas['methodName'] = 'getLog';
            $datas['fileName'] = 'Lead-Acquisition-Log-Report-';
            $datas['moduleName'] = 'Lead-Acquisition-Log';
            return $this->exportData($request, $repository, $datas);
        } catch (Throwable  | ClientException $throwable) {
            Log::info("LeadAcquisitionLogService  exportLog" . $throwable->__toString());
    ...
    public function getFilterData()
    {
        try {
            ...
            throw new Throwable(
                Log::info("Service : LeadAcquisitionLogService , Method : getFilterData : %s", $throwable->__toString())
            );
        }
    }
    /**
     ...
     * @return mixed
     */
    public function getLogList(Request $request, LeadAcquisitionLogRepository $leadAcquisitionLogRepo): mixed
    {
        try {
            $logsList = $leadAcquisitionLogRepo->getLog($request);
            return $this->responseJson(
                config('crm/http-status.success.status'),
                config('crm/http-status.success.message'),
                config('crm/http-status.success.code'),
            ...
            throw new Throwable(
                Log::info("Service : LeadAcquisitionLogService , Method : getLogList : %s", $throwable->__toString())
            );
        }
    }
}
