...
class JourneyAuthToken extends Service
{
    /**
    ...
    public function handle($request, Closure $next)
    {
        try {
            $knownRequest = false;
            // journey / web site request.
            if ($request->bearerToken() && $request->header('X-Api-Source') && $request->header('X-Api-Source-Page') && $request->header('X-Api-Type')) {
            ...
            if ($knownRequest) {
                ...
                $applicationData = $appRepo->getAuthTokenByToken($request->bearerToken());
                if ($applicationData != null && !empty($applicationData)) {
                    $request['lead_id'] = $applicationData->lead_id;
                    $path = parse_url($request->url(), PHP_URL_PATH);
                    if ($path) {
                        $segments = explode('/', rtrim($path, '/'));
                        $lastSegment = end($segments);
                        if ($lastSegment == 'application-update' && $request->quote_id) {
                            $request['quote_id'] = $request->quote_id;
                        } else {
                            $request['quote_id'] = $applicationData->quote_id;
                        }
                    } else {
                        $request['quote_id'] = $applicationData->quote_id;
                    }
            ...
            Log::info("JourneyAuthToken Exception", $th);
            return $this->responseJson(
                config('journey/http-status.unauthorized.status'),
                config('journey/http-status.unauthorized.message'),
                config('journey/http-status.unauthorized.code'),
